...
function geoFail() {
  ...
  document.querySelector(".PM10").style.backgroundColor =
    "rgba(255,255,255,0.5)";
  document.querySelector(".PM10").innerText = "미세먼지를 불러올 수 없습니다";
  document.querySelector(".PM2_5").style.backgroundColor =
    "rgba(255,255,255,0.5)";
  document.querySelector(".PM2_5").innerText =
    "초미세먼지를 불러올 수 없습니다";
}
...
function geoSuccess(position) {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      temp[1].innerHTML =
        ...
        "</strong><br>" +
        `최고기온  ${second.temp["max"]}℃` +
        "<br>" +
        `최저기온  ${second.temp["min"]}℃`;
      temp[2].innerHTML =
        "<strong>" +
  ...
  fetch(air_url)
    ...
    .then((data) => {
      ...
      const PM2_5 = parseFloat(data.list[0].components.pm2_5);
      const PM10 = parseFloat(data.list[0].components.pm10);
      document.querySelector(".PM10").innerHTML =
        "미세먼지(PM10)<br>" + `${PM10}`; //미세먼지
      document.querySelector(".PM2_5").innerHTML =
        "초미세먼지(PM2.5)<br>" + `${PM2_5}`; //초미세먼지
      if (PM10 >= 151) {
